...
import { Package, ShoppingBag, DollarSign, Users, Clock3 } from "lucide-react";
  ...
  return (
    <div className="page-enter space-y-8">
      <div className="reveal-up">
        ...
        <p className="text-muted-foreground">Ringkasan performa toko secara real-time.</p>
      </div>
      ...
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
        {metrics.map((metric, index) => (
          <Card
            key={metric.title}
            className={[
              "surface-panel reveal-up rounded-2xl border",
      ...
      <div className="grid gap-4 lg:grid-cols-7">
        <Card className="surface-panel reveal-up reveal-delay-1 rounded-2xl border lg:col-span-4">
          <CardHeader>
        ...
        <Card className="surface-panel reveal-up reveal-delay-2 rounded-2xl border lg:col-span-3">
          <CardHeader>
          ...
          <CardContent>
            <div className="space-y-3">
              {stats.recentOrders.length === 0 ? (
                <p className="py-8 text-center text-sm text-muted-foreground">Belum ada aktivitas pesanan.</p>
              ) : (
                stats.recentOrders.map((order) => (
                  <div
                    key={order.id}
                    className="rounded-xl border border-border/70 bg-card/65 px-3 py-2"
                  >
                    <div className="flex items-center gap-2">
                      <div className="flex-1 space-y-1">
                        <p className="text-sm font-medium leading-none">Pesanan baru dari {order.customerName}</p>
